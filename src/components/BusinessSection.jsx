    ...
    <div className="flex h-screen items-center bg-[#f8f9fb] py-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <div className="mb-14 text-center">
          ...
          <p className="mx-auto max-w-5xl text-base leading-8 text-gray-600 md:text-lg">
        ...
        <div className="grid gap-6 xl:grid-cols-4">
          {items.map((item) => (
            <div
              key={item.title}
              className="flex h-full flex-col rounded-3xl border border-gray-200 bg-white p-8 shadow-sm"
            >
              <div className="mb-6 h-40 overflow-hidden rounded-2xl bg-blue-100">
                <img
                  src={item.image}
                  alt={item.title}
                  className={`h-full w-full object-cover ${
                    item.title === "협업 로봇" ? "object-center" : ""
                  }`}
                />
              </div>
              <h3 className="mb-4 text-2xl font-bold text-gray-900">{item.title}</h3>
              <p className="mb-6 min-h-[96px] text-base leading-8 text-gray-600">
                {item.desc}
              </p>
              <button className="mt-auto rounded-full border border-blue-600 px-6 py-3 text-sm font-semibold text-blue-600">
                VIEW MORE
              </button>
            </div>
          ))}
